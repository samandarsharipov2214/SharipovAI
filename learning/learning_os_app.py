"""Unified SharipovAI Learning OS API and dashboard.
...
from .learning_os_core import bot_training_status, close_learning_gap, learning_os_snapshot


app = FastAPI(title="SharipovAI Learning OS")
...
def record_mistake_api(payload: dict[str, Any] = Body(default_factory=dict)) -> dict[str, Any]:
    return memory().record_mistake(
        bot=str(payload.get("bot", "learning_engine")),
        domain=str(payload.get("domain", "general")),
        mistake=str(payload.get("mistake", "unknown mistake")),
        consequence=str(payload.get("consequence", "unknown consequence")),
...
def learning_os_page() -> HTMLResponse:
    ...
    body = f"""<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>SharipovAI · Learning OS</title><style>{_css()}</style></head><body><main><section class="card"><span class="ok">LEARNING OS</span><h1>Самообучение SharipovAI</h1><p>Единый центр обучения, памяти, источников, юридического мониторинга и экзаменов ботов.</p><p><a href="/api/learning-os/snapshot">JSON snapshot</a></p></section><section class="card"><h2>Итог</h2><div class="grid"><div class="stat"><small>Ботов</small><b>{summary.get('bot_count', 0)}</b></div><div class="stat"><small>Ready</small><b>{summary.get('ready', 0)}</b></div><div class="stat"><small>Needs training</small><b>{summary.get('needs_training', 0)}</b></div><div class="stat"><small>Уроков</small><b>{summary.get('lesson_count', 0)}</b></div><div class="stat"><small>Ошибок</small><b>{summary.get('mistake_count', 0)}</b></div></div></section><section class="card"><h2>Боты</h2><table><thead><tr><th>Bot</th><th>Status</th><th>Score</th><th>Domains</th><th>Lessons</th></tr></thead><tbody>{rows}</tbody></table></section><section class="card"><h2>Последние уроки</h2><table><thead><tr><th>Bot</th><th>Domain</th><th>Lesson</th><th>Rule</th></tr></thead><tbody>{lessons}</tbody></table></section></main></body></html>"""
