...
from .evidence_policy import AgentEvidence, OutcomeEvidence
...
class OutcomeAttributionService:
    """Persist one verified outcome exactly once and update aggregate metrics."""
    ...
    def record(self, evidence: OutcomeEvidence | Mapping[str, Any]) -> dict[str, Any]:
        ...
        evidence_payload = outcome.to_dict()
        ...
        for attribution in attributions:
            self._update_agent(outcome, attribution)
    ...
    def _attribute(self, outcome: OutcomeEvidence) -> tuple[AgentAttribution, ...]:
        ...
        raw_weights = [max(item.confidence / 100.0, 0.01) * max(item.evidence_score, 0.01) for item in contributors]
        total = sum(raw_weights)
        weights = [value / total for value in raw_weights]
        result: list[AgentAttribution] = []
        contribution_by_agent = {item.agent_id: weight for item, weight in zip(contributors, weights, strict=True)}
        for agent in outcome.agents:
            ...
            expected_probability = agent.confidence / 100.0
            ...
            result.append(
                AgentAttribution(
                    ...
                    confidence_error=abs(expected_probability - observed),
    ...
    def _update_agent(self, outcome: OutcomeEvidence, attribution: AgentAttribution) -> None:
        ...
        try:
            ...
            raise RuntimeError("concurrent self-learning attribution conflict") from exc
