...
def _audit_blueprint(root: Path, record: Any) -> None:
    ...
    The function name is retained for compatibility with older tests and tools,
    but Render is no longer the deployment authority.
    """
    ...
    missing = [str(path.relative_to(root)) for path in (compose_path, caddy_path, update_path, dockerfile_path) if not path.exists()]
...
def _audit_workflows(root: Path, record: Any) -> None:
    ...
    record("ci_full_suite", "pytest" in full.lower() and "fail" in full.lower(), "full pytest gate")
    record("ci_windows_agent", "runs-on: windows-latest" in windows and "pytest" in windows.lower(), "Windows agent verification gate")
    record(
        "ci_vps_recovery",
        all(token in recovery for token in ("self-hosted", "sharipovai-ci", "127.0.0.1:8000/health", "update_from_main.sh")),
        "self-hosted diagnosis and bounded VPS recovery",
    )
