...
The runner never changes environment variables, credentials, Compose, the kill
switch or Mainnet availability. It calls canonical campaign services and writes
an append-only operator evidence bundle.
"""
...
def run(
    ...
    *,
    ...
    while str(campaign.get("status")) not in _TERMINAL:
        if cycle_count >= int(args.max_cycles) or monotonic() >= deadline:
            break
    ...
    result = _result(campaign, report, evidence_dir, cycle_count)
