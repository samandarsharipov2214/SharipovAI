...
_DANGEROUS_RULES: Final[tuple[_PatternRule, ...]] = (
    ...
    _rule(r"\bos\.chmod\s*\([\s\S]{0,200}?,\s*0o?777\s*\)", "world-writable filesystem permission"),
    _rule(r"\brm\s+-rf\s+(?:/|/\*|\$\{?HOME\}?|~)(?:\s|$)", "destructive recursive deletion"),
    _rule(r"\b(?:curl|wget)\b[^\n|]{0,500}\|\s*(?:sudo\s+)?(?:sh|bash)\b", "remote script piped to a shell"),
    ...
    _rule(r"\bEXCHANGE_LIVE_TRADING_ENABLED\s*[:=]\s*[\"']?1\b", "live trading enabled"),
    _rule(r"\bLIVE_EXECUTION_MANUAL_UNLOCK\s*[:=]\s*[\"']?1\b", "live execution unlock enabled"),
    _rule(r"\bEXECUTION_KILL_SWITCH\s*[:=]\s*[\"']?0\b", "execution kill switch disabled"),
    _rule(
...
_TEST_POLICY_WEAKENING_RULES: Final[tuple[_PatternRule, ...]] = (
    _rule(r"--(?:ignore|ignore-glob|deselect|continue-on-collection-errors)\b", "tests excluded from collection"),
...
_TEST_DEFINITION_RE: Final[re.Pattern[str]] = re.compile(r"^\s*(?:async\s+)?def\s+test_[A-Za-z0-9_]*\s*\(")
...
class SecurityGuard:
    ...
    def check(self, patch: str | bytes) -> PatchVerdict:
        """Compatibility alias for callers that use ``check`` terminology."""
    ...
    def _evaluate_file(self, file_patch: FilePatch) -> list[str]:
        ...
        if _should_scan_dangerous_content(path):
            ...
            if _unsafe_yaml_load(added_text):
                reasons.append(f"dangerous construct in {path}: yaml.load() without SafeLoader")
...
def _unsafe_yaml_load(text: str) -> bool:
    for match in re.finditer(r"\byaml\.load\s*\((?P<args>[\s\S]{0,500}?)\)", text, re.IGNORECASE):
        arguments = match.group("args")
        if not re.search(r"(?:SafeLoader|safe_load|Loader\s*=\s*yaml\.SafeLoader)", arguments):
            return True
    return False
...
def _count_matches(lines: tuple[str, ...], pattern: re.Pattern[str]) -> int:
    return sum(bool(pattern.search(line)) for line in lines if line.strip())


__all__ = ["SecurityGuard", "evaluate_patch"]
