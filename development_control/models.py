...
def _clean_unique(values: list[str] | tuple[str, ...], *, field_name: str, path_values: bool = False):
    cleaned: list[str] = []
    seen: set[str] = set()
    for value in values:
...
class CodeChangeProposal(BaseModel):
    ...
    def validate_changed_paths(cls, value: tuple[str, ...]) -> tuple[str, ...]:
        return tuple(_clean_unique(value, field_name="changed_paths", path_values=True))

    @field_validator("tests")
    @classmethod
    def validate_tests(cls, value: tuple[str, ...]) -> tuple[str, ...]:
        return tuple(_clean_unique(value, field_name="tests"))
...
class PatchVerdict(BaseModel):
    """Security Guard verdict compatible with the existing patch-validation API."""

    model_config = ConfigDict(extra="forbid", frozen=True)

    allowed: bool
    reasons: list[str] = Field(default_factory=list, max_length=200)
    policy_version: str = Field(default="development-v1", min_length=1, max_length=100)
    ...
    @field_validator("reasons", "required_checks")
    @classmethod
    def validate_text_items(cls, value: list[str], info) -> list[str]:
    ...
    def validate_verdict_contract(self) -> "PatchVerdict":
        if not self.allowed and not self.reasons:
            raise ValueError("blocked or manual-review verdicts require reasons")
        if self.allowed and self.requires_human_approval:
            raise ValueError("allowed verdict cannot require human approval")
        if self.requires_human_approval and not self.protected_paths:
            raise ValueError("human approval requires at least one protected path")
        return self
