...
The deployed commit must have complete green required CI. A queued, skipped, partial or classified failure is not approval.
...
The script creates a verified pre-transition backup, validates rendered Compose, starts the explicit Testnet override, verifies health/database state and restores production-safe mode on failure.
...
Use the dashboard at `/#campaigns` or:
...
- matched fills and remaining fills;
- actual fee total;
...
## 7. Resume after an operator-process interruption
...
The completed campaign creates a report automatically. Idempotent manual retrieval:
...
The report must state `eligible_for_manual_decision`, contain no failed campaign gates, retain its evidence SHA-256 and keep `mainnet_enabled=false`.
...
- stale/disconnected private stream;
