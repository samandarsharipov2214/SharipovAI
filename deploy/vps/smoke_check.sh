...
compose=(docker compose -f "${COMPOSE_DIR}/docker-compose.yml" --env-file "${BASE_ENV_FILE}")
if [[ "${MODE}" == "testnet-campaign" ]]; then
  ...
  compose=(docker compose -f "${COMPOSE_DIR}/docker-compose.yml" -f "${COMPOSE_DIR}/docker-compose.testnet-campaign.yml" --env-file "${CAMPAIGN_ENV_FILE}")
fi
"${validator[@]}"
...
if mode == "production":
    required.update({"EXECUTION_KILL_SWITCH": "1"})
    forbidden_true = (
