...
python3 "${COMPOSE_DIR}/validate_runtime_env.py" \
  --env-file "${BASE_ENV_FILE}" \
  --env-file "${CAMPAIGN_ENV_FILE}" \
...
rollback() {
  ...
  docker compose -f "${COMPOSE_DIR}/docker-compose.yml" --env-file "${BASE_ENV_FILE}" up -d --force-recreate --remove-orphans
  BASE_ENV_FILE="${BASE_ENV_FILE}" bash "${COMPOSE_DIR}/smoke_check.sh" production || true
...
compose=(
  ...
  -f "${COMPOSE_DIR}/docker-compose.testnet-campaign.yml"
  --env-file "${CAMPAIGN_ENV_FILE}"
)
log 'rendering explicit bounded Testnet override'
"${compose[@]}" config >/dev/null

log 'recreating application container in bounded Testnet mode'
"${compose[@]}" up -d --force-recreate --remove-orphans

BASE_ENV_FILE="${BASE_ENV_FILE}" CAMPAIGN_ENV_FILE="${CAMPAIGN_ENV_FILE}" \
  bash "${COMPOSE_DIR}/smoke_check.sh" testnet-campaign
trap - ERR
