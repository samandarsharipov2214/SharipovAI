#!/usr/bin/env python3
"""Fail-closed validation for VPS production and bounded Testnet campaign modes."""
...
def validate(values: Mapping[str, str], mode: str) -> dict[str, object]:
    ...
    if mode == "production":
        ...
        if not truthy("ALERT_DELIVERY_ENABLED"):
            warnings.append("ALERT_DELIVERY_ENABLED is off; alerts will persist but not leave the VPS")
    else:
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        required=True,
        action="append",
        help="KEY=VALUE file; later files override earlier files",
    )
    parser.add_argument("--mode", choices=("production", "testnet-campaign"), required=True)
    parser.add_argument("--json", action="store_true")
    args = parser.parse_args()
    try:
        values = merge_env(args.env_file)
        report = validate(values, args.mode)
