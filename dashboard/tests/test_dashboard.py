    ...
    class Runner:
        def run(self):
            return SimpleNamespace(
                decision="BUY", confidence=95.0, risk_level="LOW", portfolio_value=10000.0,
                paper_cash=9500.0, paper_equity=10000.0, paper_pnl=500.0,
                open_positions=1, consensus="UNANIMOUS", consensus_agreement=100.0,
                reason="Verified test runner", report="Verified test report",
                learning_summary=LearningSummary(
                    total_trades=1, wins=1, losses=0, win_rate=100.0,
                    average_profit=5.0, average_loss=0.0, best_trade=5.0,
                    worst_trade=5.0, recommendations=[],
                ),
            )
    return Runner()
...
def test_runner_failure_returns_safe_api_state_and_web2_still_loads() -> None:
    ...
    assert "SharipovAI" in page.text
