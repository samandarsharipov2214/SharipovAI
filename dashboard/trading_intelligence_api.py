...
def install_trading_intelligence_api(app: FastAPI) -> None:
    """Install trading intelligence, scoreboards, learning, and chat debug endpoints."""
    ...
    def chat_debug_get(q: str = Query(default="Что сегодня произошло?")) -> dict[str, Any]:
        return answer_chat(q, _demo_state())

    @app.post("/api/chat/debug")
    def chat_debug_post(payload: dict[str, Any] | None = Body(default=None)) -> dict[str, Any]:
        payload = payload or {}
        return answer_chat(str(payload.get("message", "")), _demo_state())

    @app.get("/chat-debug", response_class=HTMLResponse)
    def chat_debug_page(q: str = Query(default="Что сегодня произошло?")) -> HTMLResponse:
        return HTMLResponse(_render_chat_debug(q, answer_chat(q, _demo_state())))
...
def _demo_state() -> dict[str, Any]:
    return {
        ...
        "risk_level": "LOW",
        "equity": 10051.63,
        "paper_equity": 10051.63,
        "net_pnl": 51.63,
        "pnl": 51.63,
        "total_fees": 13.67,
        "commission_drag": 13.67,
        "break_even_price": 67295.4,
        "bybit_costs": {"best_trade_venue": {"best": {"product": "spot", "liquidity": "maker", "round_trip_fee": 2.0}}},
    }
...
def _css() -> str:
    return "body{margin:0;background:#070b12;color:#eef4ff;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif}main{padding:18px;max-width:1180px;margin:auto}.card{background:#111827;border:1px solid #243044;border-radius:18px;padding:16px;margin:12px 0;box-shadow:0 20px 60px rgba(0,0,0,.25)}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:10px}.stat{background:#0b1220;border:1px solid #1f2a3d;border-radius:14px;padding:12px}.stat small{display:block;color:#8ea2c4}.stat b{font-size:22px}table{width:100%;border-collapse:collapse}td,th{padding:10px;border-bottom:1px solid #243044;text-align:left;vertical-align:top}small{display:block;color:#8ea2c4;margin-top:4px}a{color:#60a5fa;font-weight:800}.ok{display:inline-block;background:#10b981;color:#03130d;border-radius:999px;padding:7px 12px;font-weight:900}.warn{display:inline-block;background:#f59e0b;color:#120a02;border-radius:999px;padding:7px 12px;font-weight:900}.bad{display:inline-block;background:#ef4444;color:#fff;border-radius:999px;padding:7px 12px;font-weight:900}pre{white-space:pre-wrap;background:#0b1220;border:1px solid #1f2a3d;border-radius:14px;padding:12px}@media(max-width:720px){table{font-size:13px}td,th{padding:8px}}"


def _render_chat_debug(question: str, answer: dict[str, Any]) -> str:
    reply = escape(str(answer.get("reply", "")))
    source_ai = escape(str(answer.get("source_ai", "unknown")))
    intent = escape(str(answer.get("intent", "unknown")))
    q = escape(question)
    body = f"""<section class="card"><span class="ok">CHAT DEBUG</span><h1>AI Chat Router Debug</h1><p>Проверка: какой внутренний AI отвечает на вопрос.</p><p><a href="/">Главная</a> · <a href="/api/chat/debug?q={q}">JSON</a></p></section><section class="card"><h2>Вопрос</h2><pre>{q}</pre></section><section class="card"><div class="grid"><div class="stat"><small>Intent</small><b>{intent}</b></div><div class="stat"><small>Source AI</small><b>{source_ai}</b></div></div></section><section class="card"><h2>Ответ</h2><pre>{reply}</pre></section><section class="card"><h2>Быстрые тесты</h2><p><a href="/chat-debug?q=Что сегодня произошло?">Что сегодня произошло?</a></p><p><a href="/chat-debug?q=Почему наблюдать?">Почему наблюдать?</a></p><p><a href="/chat-debug?q=Можно покупать BTC?">Можно покупать BTC?</a></p><p><a href="/chat-debug?q=Какие ИИ не работают?">Какие ИИ не работают?</a></p></section>"""
    return _page("Chat Debug", body)
