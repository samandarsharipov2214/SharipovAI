...
def _render(state: dict[str, Any], autorun: dict[str, Any] | None = None) -> str:
    ...
    trades = "".join(_trade_row(trade) for trade in reversed(all_trades)) or "<tr><td colspan='8'>Пока нет paper-сделок. Нажми tick.</td></tr>"
    return f"""<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>SharipovAI · Paper Activity</title><style>{_css()}</style></head><body><main><section class="card"><span class="ok">PAPER ONLY</span><h1>Paper Activity Engine</h1><p>Это активная demo/paper-симуляция. Реальные ордера заблокированы. На этой странице показываются <b>все сделки</b>, не только последние.</p><p><a href="/">Главная</a> · <a href="/api/paper-activity/state">JSON state</a></p></section><section class="card"><div class="grid"><div class="stat"><small>Trades</small><b>{summary.get('trade_count', len(all_trades))}</b></div><div class="stat"><small>Open</small><b>{summary.get('open_positions', 0)}</b></div><div class="stat"><small>Closed</small><b>{summary.get('closed_positions', 0)}</b></div><div class="stat"><small>Net PnL</small><b>{summary.get('net_pnl', 0)}</b></div><div class="stat"><small>Fees</small><b>{summary.get('total_fees', 0)}</b></div><div class="stat"><small>Last reason</small><b>{escape(str(summary.get('last_reason', '')))}</b></div><div class="stat"><small>Last tick age</small><b>{escape(str(summary.get('last_tick_age_seconds', '—')))}</b></div><div class="stat"><small>Autorun</small><b>{escape(str(autorun.get('status', 'unknown')))}</b></div><div class="stat"><small>Thread</small><b>{'alive' if autorun.get('thread_alive') else 'not alive'}</b></div></div></section><section class="card"><h2>Как ускорить</h2><p>POST <code>/api/paper-activity/tick</code> с <code>{{"force": true}}</code> принудительно делает следующий paper tick.</p><p>POST <code>/api/paper-activity/catch-up</code> догоняет пропущенные tick после ночи/сна сервера.</p><p>Env: <code>PAPER_ACTIVITY_AUTORUN_ENABLED</code>, <code>PAPER_ACTIVITY_TICK_SECONDS</code>, <code>PAPER_ACTIVITY_MAX_OPEN</code>, <code>PAPER_ACTIVITY_MAX_CATCH_UP_TICKS</code>.</p></section><section class="card"><h2>Все сделки</h2><table><thead><tr><th>#</th><th>ID</th><th>Asset</th><th>Side</th><th>Status</th><th>Notional</th><th>Net PnL</th><th>Source</th></tr></thead><tbody>{trades}</tbody></table></section></main></body></html>"""


def _trade_row(trade: dict[str, Any]) -> str:
    return f"<tr><td>{escape(str(trade.get('id', '')))}</td><td>{escape(str(trade.get('id', '')))}</td><td>{escape(str(trade.get('asset', '')))}</td><td>{escape(str(trade.get('side', '')))}</td><td>{escape(str(trade.get('status', '')))}</td><td>{escape(str(trade.get('notional', '')))}</td><td>{escape(str(trade.get('net_pnl', '')))}</td><td>{escape(str(trade.get('source', '')))}</td></tr>"
