...
_SENSITIVE_PATHS = {
    ...
    "/api/system/watchdog/tick",
    "/api/system/ai-organs/refresh",
}
_SENSITIVE_MUTATION_PREFIXES = (
    "/api/system/watchdog/reset/",
    "/api/news-agents/",
)
...
def _requires_admin(request: Request) -> bool:
    ...
    if request.method not in {"GET", "HEAD", "OPTIONS"}:
        return any(path.startswith(prefix) for prefix in _SENSITIVE_MUTATION_PREFIXES)
    return False
