...
class SystemHealthCenter:
    ...
    def snapshot(self) -> dict[str, Any]:
        ...
        if callable(put_json):
            try:
                ...
                # Health reporting must remain available even if persistence fails.
                pass
        return payload
    ...
    def _database(self) -> ComponentHealth:
        ...
        if database is None:
            return _component(
                "database",
                [],
                ["critical: canonical ProjectDatabase is absent"],
        ...
        try:
            ...
            return _component(
                "database",
                [],
                [f"critical: database health failed: {type(exc).__name__}: {exc}"],
        ...
        if health.get("status") != "ok":
            return _component(
                "database",
                [],
                [f"critical: database unavailable: {health.get('error', 'unknown error')}"],
                ["reconnect database", "restore verified backup if corruption is confirmed"],
            )
        return _component(
            "database",
            [f"backend={health.get('backend', 'unknown')}", "schema_ready"],
            [],
            [],
        )
    ...
    def _ai_organs(self) -> ComponentHealth:
        ...
        if monitor is None:
            return _component(
                "ai_organs",
                [],
                ["AI organ runtime monitor is absent"],
        ...
        try:
            ...
            return _component(
                "ai_organs",
                [],
                [f"AI organ monitor failed: {type(exc).__name__}: {exc}"],
        ...
        evidence = [f"organ_count={snapshot.get('organ_count', 0)}", f"status={status}"]
        return _component(
            "ai_organs",
            evidence,
            blockers,
            ["inspect /api/system/ai-organs"] if blockers else [],
        )
    ...
    def _market(self) -> ComponentHealth:
        ...
        if worker is None:
            return _component(
                "market",
                [],
                ["canonical market worker is absent"],
        ...
        try:
            ...
            return _component(
                "market",
                [],
                [f"market status failed: {type(exc).__name__}: {exc}"],
                ["reconnect public market stream"],
            )
        ...
        return _component(
            ...
            evidence,
            blockers,
            ["reconnect public market stream"] if blockers else [],
        )
    ...
    def _news(self) -> ComponentHealth:
        ...
        if network is None:
            return _component(
                "news",
                [],
                ["News Intelligence network is absent"],
                ["restart news runtime"],
            )
        agents = len(getattr(network, "agents", []))
        database = getattr(self.app.state, "project_database", None)
        blockers = [] if getattr(network, "database", None) is database else [
            "news memory is not using canonical database"
        ]
        return _component(
            "news",
            [f"source_agents={agents}"],
            blockers,
            ["rebind NewsHub to ProjectDatabase"] if blockers else [],
        )
    ...
    def _telegram(self) -> ComponentHealth:
        ...
        return _component(
            ...
            evidence,
            blockers,
            ["restart Telegram worker", "verify Telegram credentials"] if blockers else [],
        )
    ...
    def _security(self) -> ComponentHealth:
        ...
        return _component(
            ...
            evidence,
            blockers,
            ["enter safe mode", "restore protected execution flags"] if blockers else [],
        )
    ...
    def _storage(self) -> ComponentHealth:
        ...
        if used_percent >= self.disk_block_percent:
            return _component(
                "storage",
                evidence,
                ["critical: disk usage exceeds block threshold"],
                ["free disk space", "rotate logs and backups"],
            )
    ...
    def _backup(self) -> ComponentHealth:
        ...
        if not existing:
            return _component(
                "backup",
                [],
                ["verified backup evidence is not visible to this runtime"],
                ["run configured VPS backup job"],
            )
        newest = max(existing, key=lambda item: item.stat().st_mtime)
        age = max(0.0, self.clock() - newest.stat().st_mtime)
        max_age = _bounded_float(
            "SYSTEM_BACKUP_MAX_AGE_SECONDS",
            7200.0,
            300.0,
            604800.0,
        )
        ...
        if age > max_age:
            return _component(
                ...
                ["run verified backup immediately"],
            )
