...
def install_public_check_api(app: FastAPI) -> None:
    ...
    def check_ai() -> dict[str, object]:
        ...
        return {
            ...
            "audit_url": "/ai-audit",
            "system_audit_url": "/system-ai-audit",
            "next_urls": {"news_live": "/news-live", "ai_audit": "/ai-audit", "system_ai_audit": "/system-ai-audit", "mini_app": "/", "sources": "/api/social-news/sources", "agents": "/api/social-news/agents", "supervisor": "/api/social-news/supervisor"},
        }
    ...
    @app.get("/api/system-ai-audit")
    def system_ai_audit_api() -> dict[str, object]:
        return audit_system_ai()

    @app.get("/ai-audit", response_class=HTMLResponse)
    def ai_audit_page() -> HTMLResponse:
        return HTMLResponse(_render_audit_page())

    @app.get("/system-ai-audit", response_class=HTMLResponse)
    def system_ai_audit_page() -> HTMLResponse:
        return HTMLResponse(_render_system_audit_page())
    ...
    def news_live_api() -> dict[str, object]:
        ...
        agents = run_news_agents()
        news = analyzed_news_payload()
        return {"status": "ok", "sources": {"total": sources.get("total", 0), "categories": sorted((sources.get("by_category") or {}).keys())}, "supervisor": agents.get("supervisor", {}), "agents": agents.get("agents", []), "hot_news": news.get("items", [])[:12], "summary": news.get("summary", {})}


def _render_system_audit_page() -> str:
    ...
    rows = "".join(_system_audit_row(item) for item in interviews)
    actions = "".join(f"<li>{escape(str(action))}</li>" for action in audit.get("priority_actions", []))
    return f"""<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>SharipovAI · Полный аудит ИИ</title><style>{_audit_css()}</style></head><body><main><section class="card"><span class="ok">SYSTEM AI AUDIT</span><h1>Беседа со всеми ИИ-ботами</h1><p>{escape(str(auditor.get('summary', '')))}</p><p><a href="/">Главная</a> · <a href="/news-live">Новости</a> · <a href="/ai-audit">Только News AI</a> · <a href="/api/system-ai-audit">JSON аудит</a></p></section><section class="card"><h2>{escape(str(auditor.get('name', 'System AI Auditor')))}</h2><div class="grid"><div class="stat"><small>Оценка</small><b>{escape(str(auditor.get('overall_grade', 'UNKNOWN')))}</b></div><div class="stat"><small>Работают</small><b>{auditor.get('working', 0)} / {auditor.get('total', 0)}</b></div><div class="stat"><small>Частично/недоработаны</small><b>{auditor.get('partial_or_underbuilt', 0)}</b></div><div class="stat"><small>Делают вид/заглушки</small><b>{auditor.get('fake_like', 0)}</b></div></div></section><section class="card"><h2>Вердикт по каждому ИИ</h2><table><thead><tr><th>AI</th><th>Зона</th><th>Вердикт</th><th>Health</th><th>Доказательства</th><th>Проблемы</th><th>Что делать</th></tr></thead><tbody>{rows}</tbody></table></section><section class="card"><h2>Приоритет доработок</h2><ol>{actions}</ol></section></main></body></html>"""
...
def _render_audit_page() -> str:
    ...
    actions = "".join(f"<li>{escape(str(action))}</li>" for action in audit.get("priority_actions", []))
    return f"""<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>SharipovAI · AI Audit</title><style>{_audit_css()}</style></head><body><main><section class="card"><span class="ok">AI AUDIT</span><h1>Беседа с News AI</h1><p>{escape(str(auditor.get('summary', '')))}</p><p><a href="/system-ai-audit">Полный аудит всех ИИ</a> · <a href="/news-live">Новости</a> · <a href="/api/ai-audit">JSON аудит</a></p></section><section class="card"><h2>{escape(str(auditor.get('name', 'AI Auditor')))}</h2><div class="grid"><div class="stat"><small>Оценка</small><b>{escape(str(auditor.get('overall_grade', 'UNKNOWN')))}</b></div><div class="stat"><small>Работают</small><b>{auditor.get('working', 0)} / {auditor.get('total', 0)}</b></div><div class="stat"><small>Недоработаны</small><b>{auditor.get('underbuilt', 0)}</b></div><div class="stat"><small>Делают вид/заглушки</small><b>{auditor.get('fake_like', 0)}</b></div></div></section><section class="card"><h2>Итог беседы с каждым News AI</h2><table><thead><tr><th>AI</th><th>Вердикт</th><th>Health</th><th>Источники</th><th>Проблемы</th><th>Что делать</th></tr></thead><tbody>{rows}</tbody></table></section><section class="card"><h2>Приоритет доработок</h2><ol>{actions}</ol></section></main></body></html>"""


def _audit_css() -> str:
    return "body{margin:0;background:#070b12;color:#eef4ff;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif}main{padding:18px;max-width:1180px;margin:auto}.card{background:#111827;border:1px solid #243044;border-radius:18px;padding:16px;margin:12px 0;box-shadow:0 20px 60px rgba(0,0,0,.25)}.grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.stat{background:#0b1220;border:1px solid #1f2a3d;border-radius:14px;padding:12px}.stat small{display:block;color:#8ea2c4}.stat b{font-size:22px}table{width:100%;border-collapse:collapse}td,th{padding:10px;border-bottom:1px solid #243044;text-align:left;vertical-align:top}small{display:block;color:#8ea2c4;margin-top:4px}a{color:#60a5fa;font-weight:800}.ok{display:inline-block;background:#10b981;color:#03130d;border-radius:999px;padding:7px 12px;font-weight:900}.warn{display:inline-block;background:#f59e0b;color:#120a02;border-radius:999px;padding:7px 12px;font-weight:900}.bad{display:inline-block;background:#ef4444;color:#fff;border-radius:999px;padding:7px 12px;font-weight:900}@media(max-width:720px){.grid{grid-template-columns:1fr}table{font-size:13px}td,th{padding:8px}}"


def _system_audit_row(item: dict[str, Any]) -> str:
    verdict = str(item.get("verdict", "unknown"))
    css = "ok" if verdict == "работает" else "bad" if verdict in {"делает вид", "заглушка"} else "warn"
    evidence = "<br>".join(escape(str(entry)) for entry in item.get("evidence", []))
    problems = "<br>".join(escape(str(problem)) for problem in item.get("problems", []))
    return f"<tr><td><b>{escape(str(item.get('name', 'AI')))}</b><small>{escape(str(item.get('id', '')))}</small></td><td>{escape(str(item.get('scope', 'system')))}</td><td><span class='{css}'>{escape(verdict)}</span></td><td>{escape(str(item.get('health_score', 0)))}%</td><td><small>{evidence}</small></td><td><small>{problems}</small></td><td>{escape(str(item.get('next_fix', '')))}</td></tr>"
...
def _audit_row(item: dict[str, Any]) -> str:
    ...
    css = "ok" if verdict == "работает" else "bad" if verdict in {"делает вид", "заглушка"} else "warn"
    problems = "<br>".join(escape(str(problem)) for problem in item.get("problems", []))
    return f"<tr><td><b>{escape(str(item.get('name', 'AI')))}</b><small>{escape(str(item.get('id', '')))}</small></td><td><span class='{css}'>{escape(verdict)}</span></td><td>{escape(str(item.get('health_score', 0)))}%</td><td>{escape(str(item.get('source_count', 0)))} / items {escape(str(item.get('item_count', 0)))}</td><td><small>{problems}</small></td><td>{escape(str(item.get('next_fix', '')))}</td></tr>"
