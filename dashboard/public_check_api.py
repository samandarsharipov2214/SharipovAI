...
def install_public_check_api(app: FastAPI) -> None:
    """Install public AI check and live news endpoints."""
...
<head>
  ...
  <style>
    ...
    h1{{font-size:28px;margin:0 0 8px}} h2{{font-size:20px;margin:0 0 10px}} p{{line-height:1.45}}
    .ok{{display:inline-block;background:#10b981;color:#03130d;border-radius:999px;padding:7px 12px;font-weight:900}}
    .warn{{display:inline-block;background:#f59e0b;color:#120a02;border-radius:999px;padding:7px 12px;font-weight:900}}
    ...
    .news-grid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(250px,1fr));gap:10px}}
    .news{{background:#0b1220;border:1px solid #1f2a3d;border-radius:16px;padding:13px}}
    .news b{{display:block;margin-bottom:8px}} .news small{{color:#8ea2c4;display:block;line-height:1.4}}
    ...
    a{{color:#60a5fa;font-weight:800}} .topnav{{display:flex;gap:12px;flex-wrap:wrap;margin-top:12px}}
    @media(max-width:720px){{.grid{{grid-template-columns:1fr}} table{{font-size:13px}} td,th{{padding:8px}}}}
  </style>
</head>
<body>
<main>
  <section class="card">
    ...
    <h1>{page_title}</h1>
    <p>{intro}</p>
    <div class="topnav"><a href="/">Главная</a><a href="/news-live">Новости</a><a href="/check-ai">Проверить ИИ</a><a href="/api/news-live">JSON</a></div>
  </section>
  <section class="card">
    <h2>{escape(str(supervisor.get('name', 'Main News Supervisor AI')))}</h2>
    <div class="grid">
      <div class="stat"><small>Решение</small><b>{escape(str(supervisor.get('decision', 'UNKNOWN')))}</b></div>
    ...
    <p>{escape(str(supervisor.get('assessment', '')))}</p>
  </section>
  <section class="card">
    <h2>Самые обсуждаемые новости</h2>
    <div class="news-grid">{hot_cards}</div>
  </section>
  <section class="card">
    <h2>Подсистемные News AI-боты</h2>
    <table><thead><tr><th>AI</th><th>Статус</th><th>Health</th><th>Источники</th><th>Достоверность</th></tr></thead><tbody>{agent_rows}</tbody></table>
  </section>
  <section class="card">
    <h2>Что доказывает, что ИИ живой</h2>
    <p>Если здесь меняются новости, есть источники, есть под-AI и главный Supervisor выдаёт решение — система работает. Если какой-то под-AI покажет attention/overloaded, главный ИИ отразит это в решении.</p>
    <p><a href="/api/social-news/sources">Источники</a> · <a href="/api/social-news/agents">Агенты</a> · <a href="/api/social-news/supervisor">Supervisor</a></p>
  </section>
</main>
</body>
</html>"""
...
def _agent_row(agent: dict[str, Any]) -> str:
    ...
    return (
        f"<tr><td><b>{escape(str(agent.get('name', 'AI')))}</b><small>{escape(str(agent.get('responsibility', '')))}</small></td>"
        f"<td><span class='{badge}'>{status}</span></td>"
        f"<td>{escape(str(agent.get('health_score', 0)))}%</td>"
        f"<td>{escape(str(agent.get('source_count', 0)))}</td>"
        f"<td>{escape(str(agent.get('average_credibility_percent', 0)))}%</td></tr>"
    )
