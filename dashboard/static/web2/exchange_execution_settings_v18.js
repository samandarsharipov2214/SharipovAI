(() => {
  ...
  const time = value => { const d=new Date(value||0); return Number.isNaN(d.getTime())?'—':d.toLocaleString('ru-RU'); };
  ...
  async function trades(){
    ...
    } catch { out.innerHTML=title('Сделки','Журнал недоступен')+empty('Биржа не вернула историю операций.'); }
  }
  ...
  async function virtualAccount(){
    ...
    try{
      const d=await request('/api/virtual-account/state'), items=list(d.trades,d.orders,d.history);
