...
def parse_development_callback(data: str) -> DevelopmentCallback:
    ...
    if len(parts) == 4 and parts[0] == "devfix" and parts[1] in {"a", "r"}:
        ...
        return DevelopmentCallback(
            "approve" if parts[1] == "a" else "reject",
            _short_id(parts[2]),
            token,
        )
...
def handle_development_callback(
    callback_query: Mapping[str, Any],
    *,
    controller: Any | None = None,
) -> dict[str, Any]:
    """Validate one callback and queue an explicitly approved patch for host use."""
    ...
    if approved:
        decision = active_controller.queue_host_application(decision.decision_id)
        return {"status": "queued_for_host", "decision": decision.to_dict()}
