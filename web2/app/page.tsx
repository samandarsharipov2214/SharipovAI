    ...
    <main className="shell">
      ...
      <section className="workspace">
        <header className="topbar">
          <div className="welcome"><p>Привет! 👋</p><span>SharipoAI работает на полную мощность</span></div>
...
function NewsPage({rows}:{rows:Json[]}) {
  ...
  if (!list.length) {
    return <article className="panel newsCard"><Newspaper/><div><small>Новости</small><h3>Нет подтверждённых новостей</h3><p>Интерфейс не подставляет демонстрационные события вместо данных API.</p></div></article>;
  }
