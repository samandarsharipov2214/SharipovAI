...
def bind_preview_to_candidate(
    ...
    *,
    ...
    preview_tokens = [
        item
        for item in candidate.signal_evidence
    ...
    effective_rules_age = min(max(int(max_instrument_rules_age_ms), 1_000), 300_000)
    if now_ms <= 0:
...
def _same(left: float, right: float) -> bool:
    return _finite(left) and _finite(right) and isclose(
        float(left),
        float(right),
        rel_tol=1e-9,
        abs_tol=1e-9,
