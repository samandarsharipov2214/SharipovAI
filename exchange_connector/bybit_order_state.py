...
class BybitOrderStateStore:
    ...
    def __init__(
        ...
        *,
        ...
        self.environment = str(configured).strip().lower()
        if self.environment not in _ALLOWED_ENVIRONMENTS:
            raise ValueError("environment must be paper, testnet or mainnet")
        ...
        with self._lock:
            ...
            for index, raw in enumerate(data):
                try:
                    ...
                    existing = orders.get(existing_key) if existing_key else None
                    outcome = _apply_state(existing, state)
                    storage_key = existing_key or _new_storage_key(state)
                    if outcome == "duplicate":
                        duplicates.append(storage_key)
                    else:
                        orders[storage_key] = state.to_dict()
    ...
    for item in tracked_orders:
        ...
        if order_id:
            if order_id in by_order_id:
                raise ValueError("duplicate order_id in tracker snapshot")
            by_order_id[order_id] = item
        if link_id:
            if link_id in by_link_id:
                raise ValueError("duplicate order_link_id in tracker snapshot")
...
def _business_mismatches(journal: Mapping[str, Any], tracked: Mapping[str, Any]) -> list[str]:
    mismatches: list[str] = []
    expected_environment = _normalize_environment(journal.get("environment"))
    if expected_environment != str(tracked.get("environment", "")):
    ...
    if str(journal.get("side", "")).strip().title() != str(tracked.get("side", "")).strip().title():
...
def _normalize_environment(value: Any) -> str:
    ...
    mapping = {"sandbox": "testnet", "live": "mainnet"}
