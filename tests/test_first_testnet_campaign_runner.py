...
class FakeReports:
    def generate(self, campaign_id, *, actor):
        del actor
        return {
            "report_id": f"report_{campaign_id}",
            "eligible_for_manual_decision": True,
...
def test_runner_collects_terminal_real_fill_evidence_bundle(tmp_path) -> None:
    ...
    services = SimpleNamespace(
        ...
        campaign=FakeCampaign([running, _completed()]),
        reports=FakeReports(),
    )
...
def test_runner_does_not_start_when_plan_is_blocked(tmp_path) -> None:
    operations = FakeOperations(
        {"can_start": False, "blockers": ["private_stream_ready"]},
        {},
    )
    services = SimpleNamespace(
        operations=operations,
...
def test_runner_can_resume_the_only_active_campaign_but_not_bypass_other_blockers(tmp_path) -> None:
    running = {
    ...
    operations = FakeOperations(
        {"can_start": False, "blockers": ["no_active_campaign"]},
        {},
    )
    services = SimpleNamespace(
        operations=operations,
        campaign=FakeCampaign([running, _completed("campaign_resume")]),
        reports=FakeReports(),
    )
    result = run(
        _args(tmp_path, resume_campaign_id="campaign_resume"),
        services,
        sleep=lambda _: None,
        monotonic=lambda: 0.0,
    )
    ...
    blocked_operations = FakeOperations(
        {"can_start": False, "blockers": ["no_active_campaign", "private_stream_ready"]},
        {},
    )
