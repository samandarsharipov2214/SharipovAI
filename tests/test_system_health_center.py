...
def test_missing_kill_switch_blocks_system_even_with_other_components_healthy(tmp_path: Path, monkeypatch) -> None:
    app = healthy_app(tmp_path, monkeypatch)
...
def test_recovery_plan_is_advisory_and_never_financially_automatic(tmp_path: Path, monkeypatch) -> None:
    app = healthy_app(tmp_path, monkeypatch)
