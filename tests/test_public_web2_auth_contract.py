...
def test_public_verifier_checks_the_site_v1_public_entry() -> None:
    source = VERIFY.read_text(encoding="utf-8")

    assert 'public_root_status="$(' in source
    assert 'if [[ "$public_root_status" != "200"' in source
    assert "PUBLIC_SITE_V1_ENTRY_OK" in source
    assert "PUBLIC_SITE_V1_ENTRY_FAILED" in source
...
def test_public_verifier_checks_static_web2_shell_not_private_root() -> None:
    ...
    assert '"$PUBLIC_URL/static/site-v1/index.html"' in source
    assert 'public_static_status="$(' in source
    assert 'if [[ "$public_static_status" != "200" ]]' in source
    assert "PUBLIC_SITE_V1_STATIC_HTTP_STATUS_FAILED" in source
    assert "PUBLIC_SITE_V1_STATIC_HTTP_STATUS_OK" in source
    assert "PUBLIC_SITE_V1_ASSET_CONTRACT_FAILED" in source
    assert "PUBLIC_SITE_V1_ASSET_CONTRACT_OK" in source
...
def test_auth_guard_remains_fail_closed_for_protected_routes() -> None:
