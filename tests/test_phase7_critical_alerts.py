...
def _snapshot(*, active=None, latest=None, active_count=0, errors=None):
    return {
        ...
        "plan": {
            "execution": {"kill_switch": False, "restart_safe": True, "status": "ok"},
            "private_stream": {"ready": True, "status": "ready"},
        },
        "phase7_monitor": {"heartbeat_stale": False},
        "orchestrator": {"errors": list(errors or [])},
    }


def test_critical_alerts_are_deduplicated_and_resolved(tmp_path) -> None:
    ...
    snapshot["plan"]["execution"] = {"kill_switch": True, "restart_safe": False, "status": "blocked"}
    snapshot["plan"]["private_stream"] = {"ready": False, "status": "stale"}

    first = service.evaluate(snapshot, now_ms=1_000_000)
    assert {item["code"] for item in first["open_alerts"]} == {
...
def test_blocked_campaign_heartbeat_and_orchestrator_are_critical(tmp_path) -> None:
    service = CampaignCriticalAlertService(
        _database(tmp_path),
    ...
    snapshot = _snapshot(
        ...
        errors=["schedule_1: reconciliation failed"],
    )
    snapshot["phase7_monitor"] = {"heartbeat_stale": True, "heartbeat_age_seconds": 61.0}
    result = service.evaluate(snapshot, now_ms=2_000_000)
    assert {item["code"] for item in result["open_alerts"]} == {
        "phase7_monitor_heartbeat_stale",
        "campaign_blocked",
        "campaign_evidence_integrity_failure",
        "campaign_orchestrator_failure",
    }
    assert result["critical_open_count"] == 3


def test_alert_monitor_is_read_only_and_reports_status(tmp_path, monkeypatch) -> None:
    ...
    monitor = CampaignCriticalAlertMonitor(
        provider,
        CampaignCriticalAlertService(_database(tmp_path), delivery=lambda record: {"status": "captured"}),
    )
    result = monitor.tick()
    assert calls == 1
