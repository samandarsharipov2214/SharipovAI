...
def test_backup_helper_uses_read_only_named_volume_and_no_network() -> None:
    ...
    assert '"$image_name" - "$source_mode" "$MIN_FREE_BYTES" "$RESERVE_BYTES" "$snapshot_format" <<\'PY\'' in source
...
def test_backup_uses_sqlite_snapshot_and_forbids_symlinks() -> None:
    ...
    assert 'sqlite3.connect(db.resolve().as_uri() + "?mode=ro", uri=True)' in source
