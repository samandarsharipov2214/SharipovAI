...
from development_control.general_controller import AgentDecision
...
class Controller:
    ...
    def get(self, short_id: str) -> AgentDecision:
        assert short_id == self.decision.short_id
        return self.decision

    def decide(self, *args):
        self.calls.append(args)
        self.decision.status = "owner_approved" if args[1] else "rejected"
        return self.decision
...
def test_callback_handler_passes_verified_identity_and_token() -> None:
    item = decision()
    ...
    assert service.calls == [
        (item.short_id, True, "12345", "12345", "approval-token", "telegram_inline_approve")
    ]
...
def test_info_callback_is_read_only() -> None:
    ...
    result = handle_development_callback(
        {"data": f"devfix:i:{item.short_id}", "from": {"id": 12345}, "message": {"chat": {"id": 12345}}},
        controller=service,
    )
