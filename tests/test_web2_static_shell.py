...
def test_web2_files_exist_and_are_nonempty() -> None:
    for path in (INDEX, SHELL, STYLE, TRUTH_GUARD):
...
def test_all_required_navigation_sections_are_present() -> None:
    ...
    required = {
        "overview": "Обзор", "market": "Рынок", "decision": "Решение ИИ",
        "portfolio": "Портфель", "trades": "Сделки", "bots": "Центр ИИ",
        "chat": "ИИ-чат", "news": "Новости", "risk": "Центр рисков", "bybit": "Bybit",
        "learning": "Центр обучения", "control": "Главное управление",
        "evidence": "Хранилище доказательств", "virtual": "Виртуальный счёт",
        "campaigns": "Кампании", "reports": "Отчёты", "settings": "Настройки",
    }
...
def test_brand_spelling_and_no_old_name() -> None:
    combined = "\n".join(path.read_text(encoding="utf-8") for path in (INDEX, SHELL))
...
def test_ui_has_resilient_truthful_fallback_content() -> None:
    ...
    assert "Math.random" not in shell
...
def test_single_service_asset_paths_and_no_cache_host() -> None:
    ...
    assert "/static/web2/web2_shell_v44.js?" in html
    assert "/static/web2/overview_runtime_v44.js?" in html
