...
def test_full_audit_is_ready_deterministic_and_mainnet_false(tmp_path, monkeypatch):
...
def test_audit_blocks_live_execution_and_invalid_notional(tmp_path, monkeypatch):
    environment = _safe_environment(monkeypatch, tmp_path)
...
def test_audit_blocks_database_failure(tmp_path, monkeypatch):
    ...
    def failed_health(_self):
        return {"status": "error", "backend": "postgresql", "error": "connection failed"}

    monkeypatch.setattr("storage.ProjectDatabase.health", failed_health)
    report = ProductionAudit(ROOT, environ=environment).run()
    assert report["status"] == "blocked"
    assert "canonical_database_health" in report["blockers"]
    check = next(item for item in report["checks"] if item["name"] == "canonical_database_health")
