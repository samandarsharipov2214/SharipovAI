...
def test_missing_or_failed_fixer_returns_none() -> None:
    assert route_successful_ai_fix(SimpleNamespace(), head="a", output="failed", command=[]) is None

    class FailedFixer:
        def attempt(self, **kwargs):
            return {"success": False, "patch": "diff --git a/a b/a"}

    assert route_successful_ai_fix(SimpleNamespace(ai_fixer=FailedFixer()), head="a", output="failed", command=[]) is None
