...
def test_runtime_env_validator_accepts_safe_production_and_separate_campaign_overlay(tmp_path) -> None:
    ...
    assert code == 0
    assert production["safe_to_continue"] is True

    code, campaign = _run_validator(base, overlay, mode="testnet-campaign")
    assert code == 0
    assert campaign["safe_to_continue"] is True
    assert campaign["mainnet_enabled"] is False


def test_runtime_env_validator_rejects_mainnet_credentials_and_unsafe_caps(tmp_path) -> None:
    base = tmp_path / ".env.vps"
    overlay = tmp_path / ".env.testnet-campaign"
...
def test_phase7_shell_scripts_have_valid_syntax_and_exact_confirmations() -> None:
    scripts = [
        DEPLOY / "smoke_check.sh",
        DEPLOY / "testnet_campaign_deploy.sh",
        DEPLOY / "testnet_campaign_stop.sh",
    ]
    ...
    assert deploy.index("export_backup.sh") < deploy.index("up -d --force-recreate")
    assert "restoring production-safe compose" in deploy


def test_compose_contract_keeps_mainnet_off_and_injects_testnet_secrets_only_in_override() -> None:
    ...
    assert 'EXCHANGE_LIVE_TRADING_ENABLED: "0"' in base
    ...
    assert 'FEATURE_BYBIT_LIVE_EXECUTION: "0"' in override
