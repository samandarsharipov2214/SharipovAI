...
def test_legacy_json_migrates_once_without_duplicates(tmp_path: Path) -> None:
    ...
    marker = db.get_json("migrations", first.migration_key)
    assert marker and marker["value"]["completed"] is True
...
def test_journal_does_not_truncate_and_same_path_instances_share_records(tmp_path: Path) -> None:
    db = database(tmp_path)
    path = tmp_path / "journal.json"
    first = ExecutionJournal(str(path), database=db)
    second = ExecutionJournal(str(path), database=db)
...
def test_different_journal_paths_are_isolated(tmp_path: Path) -> None:
    db = database(tmp_path)
    first = ExecutionJournal(str(tmp_path / "a.json"), database=db)
    second = ExecutionJournal(str(tmp_path / "b.json"), database=db)
    first.append({"journal_event_id": "one", "status": "accepted"})
    assert len(first.load()["orders"]) == 1
    assert second.load()["orders"] == []
