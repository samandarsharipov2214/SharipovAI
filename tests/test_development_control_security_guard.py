...
def test_safe_source_patch_is_allowed() -> None:
    verdict = evaluate_patch(_replace_patch("src/math_utils.py", "VALUE = 1", "VALUE = 2"))
    assert verdict == PatchVerdict(allowed=True, reasons=[])
    assert verdict.to_dict() == {"allowed": True, "reasons": []}
...
def test_dangerous_added_constructs_are_denied(path: str, added: str, fragment: str) -> None:
    verdict = evaluate_patch(_replace_patch(path, "safe = True", added))
    assert verdict.allowed is False
    assert fragment.casefold() in _reasons(_replace_patch(path, "safe = True", added)).casefold()
...
def test_skip_or_xfail_cannot_be_added_to_tests() -> None:
    patch = """diff --git a/tests/test_risk.py b/tests/test_risk.py
index 1111111..2222222 100644
--- a/tests/test_risk.py
...
"""
    verdict = evaluate_patch(patch)
    assert verdict.allowed is False
    assert "skip/xfail marker added" in _reasons(patch)
...
def test_plain_unified_diff_is_supported() -> None:
...
def test_malformed_or_empty_patch_is_denied() -> None:
