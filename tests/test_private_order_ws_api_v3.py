...
class Worker:
    def __init__(self, fail_snapshot=False): self.started = 0; self.stopped = 0; self.fail_snapshot = fail_snapshot
    def start(self): self.started += 1
    def stop(self): self.stopped += 1
    def status(self): return {"enabled": False, "connected": False, "credential_profile": "testnet_execution"}
    def snapshot(self):
        if self.fail_snapshot: raise RuntimeError("corrupt state")
        return {"status": "unverified", "orders": []}
    def reconcile(self, journal): return {"status": "blocked", "restart_safe": False, "errors": ["not verified"], "journal": journal}


class Journal:
    def load(self): return {"orders": [{"order_link_id": "sai_x"}]}
...
def test_anonymous_non_admin_and_admin_paths(monkeypatch) -> None:
    ...
    app = FastAPI(); worker = Worker(); install_private_order_ws_api(app, worker=worker, journal=Journal())
...
def test_snapshot_error_and_idempotent_installer(monkeypatch) -> None:
    ...
    monkeypatch.setattr(dashboard_app, "_session_username", lambda request: "admin")
    monkeypatch.setattr(dashboard_app, "_is_admin_request", lambda request: True)
    app = FastAPI(); install_private_order_ws_api(app, worker=Worker(True), journal=Journal())
