...
export function useChat() {
  ...
  const sendMessage = useCallback(
    async (rawText: string): Promise<boolean> => {
      ...
      if (previous && previous.text === text && now - previous.at < 700) {
        return false;
      }
      ...
      dispatch({ type: "append", messages: [userMessage, pendingMessage] });
      ...
      try {
        const response = await requestChat(
          {
            message: text,
            history: requestHistory(state.messages),
          },
          controller.signal,
        );
  ...
  const retryLast = useCallback(async () => {
    if (!state.lastUserMessage) return false;
    return sendMessage(state.lastUserMessage);
